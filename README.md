...
- RenderでBackendがスリープしている状態でも、Frontendからの `/health` リクエストで起動が始まる
- 起動完了後、Frontendの起動待ち画面から通常チャットUIへ自動復帰する
- Renderプロジェクトをpauseした状態では接続失敗として扱われる
- Renderプロジェクトをresumeした後、Frontendの再試行操作で通常利用へ復帰できる
...
- Renderスリープ復帰時にFrontendからの `/health` でBackendが起動すること
